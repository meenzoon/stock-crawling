...
CLI 진입점은 `stock_crawler/__main__.py` → `stock_crawler.cli:app` 입니다.
호출은 `python -m stock_crawler` 로 합니다.
...
uv run python -m stock_crawler tickers --market nasdaq --top 200 --refresh
...
uv run python -m stock_crawler fetch --market nasdaq --top 200 --refresh-tickers
...
uv run python -m stock_crawler schedule --market nasdaq --top 200 --interval-hours 6 --run-now
```
